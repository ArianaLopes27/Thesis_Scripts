...
cfg.method  ='coh';
cfg.complex = 'absimag';
source_conn_ASD = ft_connectivityanalysis(cfg, source);  %theta
...
cfg.threshold=.1
network_full  = ft_networkanalysis(cfg,source_conn_ASD); %theta

% sourceinterpolate
cfg           = [];
cfg.parameter = 'degrees';
network_int   = ft_sourceinterpolate(cfg, network_full, dkatlas);
network_int_ASD   = ft_sourceparcellate([], network_int, dkatlas);  %theta
...
cfg.colorbar     = 'no';
figure(10);
ft_sourceplot(cfg, network_int_ASD);  %thea
view([-90 30]);
